...
bool Valid(double[,] arr)
{
    if (arr[0, 1] == arr[0, 2])
    {
        if (arr[1, 1] == arr[1, 2]) { Console.Write("Заданные прямые совпадают!"); return false; }
        else { Console.Write("Заданные прямые паралельны!"); return false; }
    }
    return true;
}
